    ...
    public class MovingSystem : IUpdate
    {
        public override void Update(GameTime gameTime)
        {
            ...
            foreach(var entity in EntityDict)
            {
                ...
                {
                    if (phys != null)
                        move += Vector3.Forward + new Vector3(0,0,-(phys.Mass * phys.Acceleration.Z) * (float)gameTime.ElapsedGameTime.TotalSeconds);
                    else
                ...
                {
                    if (phys != null)
                        move += Vector3.Backward + new Vector3(0, 0, (phys.Mass * phys.Acceleration.Z) * (float)gameTime.ElapsedGameTime.TotalSeconds);
                    else
                        trans.Position -= trans.Forward * (float)gameTime.ElapsedGameTime.TotalSeconds * 2;
                ...
                {
                    if (phys != null)
                        move += Vector3.Left + new Vector3(-(phys.Mass * phys.Acceleration.X) * (float)gameTime.ElapsedGameTime.TotalSeconds, 0, 0);
                    else
                ...
                {
                    if (phys != null)
                        move += Vector3.Right + new Vector3((phys.Mass * phys.Acceleration.X) * (float)gameTime.ElapsedGameTime.TotalSeconds, 0, 0);
                    else
                        trans.Position += trans.Right * (float)gameTime.ElapsedGameTime.TotalSeconds * 2;
                ...
                {
                    if (phys != null)
                        move += Vector3.Up + new Vector3(0, (phys.Mass * phys.Acceleration.X) * (float)gameTime.ElapsedGameTime.TotalSeconds, 0) * PhysicsComponent.DEFAULT_LEGFORCE/1000;
